...
Настройки игры по умолчанию ожидают `deepseek-r1:8b` и `gemma3:4b`. При открытии AI Settings приложение само вызывает `/api/tags` и заполняет шесть списков: решения и рапорты штаба, закупок и кораблей. Кнопка «Обновить модели» повторяет запрос. Другие установленные модели можно выбрать независимо для каждого этапа.
...
- шесть model-select заполнены результатом `/api/tags`;
