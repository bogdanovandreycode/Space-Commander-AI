...
- `src/game` — состояние, правила, legal actions и победа;
- `src/ai` — агенты и fallback;
- `src/services` — Ollama, localStorage и diagnostics;
- `src/render/pixi` — только игровое поле;
- `src/ui` — DOM-интерфейс и локализация;
- `doc` — конфигурации, дизайн-документы и канон;
- `old version` — read-only исторические реализации.

Игровые числа читаются только из `doc/configs`. Production bundle включает их на этапе сборки и не загружает `doc` во время выполнения.
...
Тесты не обращаются к реальному Ollama.
