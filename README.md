...
По умолчанию приложение обращается к `http://localhost:11434/api/chat`, а список установленных моделей получает через `/api/tags`. Для каждой роли есть независимая пара моделей:

- решение штаба: `deepseek-r1:8b`, резервный штаб без reasoning: `gemma3:4b`, рапорт штаба: `gemma3:4b`;
- решение закупок и экономический рапорт: `gemma3:4b`;
- решение корабля и его художественный рапорт: `gemma3:4b`.
...
При открытии Settings модели загружаются из Ollama и сортируются по имени. Сохранённая, но отсутствующая модель остаётся в списке с пометкой «недоступна», поэтому временная ошибка сервиса не стирает настройки.

Если основной штаб не отвечает или возвращает некорректный результат, приложение один раз обращается к выбранной резервной модели штаба с принудительным `think: false`. Если недоступна и она, командная связь считается потерянной: Procurement продолжает оценивать экономику, а Unit Agents действуют самостоятельно без штабных рекомендаций, используя локальную обстановку и актуальные legal actions.

## Игровой интерфейс

Левая панель показывает только состояние игрока. Правая панель контекстная:

- своя планета открывает параметры колонии и её верфь;
- вражеская планета показывает экономическую разведку;
- дружественный корабль показывает историю действий по секторам;
- вражеский корабль показывает персональный дневник AI-рапортов;
- если корабль находится над планетой, сначала предлагается выбрать нужный объект.

Кнопки `AI Activity` и «Журнал событий» открывают отдельные диалоги. Hover над сектором на desktop показывает имена и lore всех объектов; на touch ту же роль выполняет контекстная карточка.
...
Корабли и планеты получают уникальные фракционные имена из `names.json`. Генерация детерминирована сохранённым seed, поэтому имена не меняются после загрузки. Новая колония сразу получает активную верфь; её ремонт впервые выполняется в ближайшем начале собственного хода, а доход подключается со следующего собственного хода.

## Сохранения

Матч автоматически сохраняется в localStorage. Главное меню позволяет продолжить, начать заново или очистить сохранение. Формат остаётся `saveVersion: 1`: старые поля готовности колонии, имена и прежнее поле `report` безопасно мигрируют при загрузке. Несовместимая версия сохранения не загружается.
...
Все тесты находятся в `unit`, зеркалируют `gamecore`/`engine` и не обращаются к реальному Ollama. UI проверяется в jsdom, а AI и Ollama — через управляемые mock-клиенты.
